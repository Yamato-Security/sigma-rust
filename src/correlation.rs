use crate::{Event, Rule, rule_from_yaml};
...
impl CorrelationCondition {
    pub fn matches(&self, value: u64) -> bool {
        self.gte.is_none_or(|gte| value >= gte)
            && self.lte.is_none_or(|lte| value <= lte)
            && self.eq.is_none_or(|eq| value == eq)
            && self.gt.is_none_or(|gt| value > gt)
            && self.lt.is_none_or(|lt| value < lt)
    }
}
...
impl CorrelationEngine {
    ...
    fn process_event_count<'a>(
        ...
        for (key, bucket_events) in buckets {
            ...
            let matched = rule
                ...
                .condition
                .as_ref()
                .is_none_or(|condition| condition.matches(count));
    ...
    fn process_value_count<'a>(
        ...
        let field = condition.and_then(|c| c.field.as_ref()).ok_or_else(|| {
            anyhow!("value_count correlation requires 'field' parameter in condition")
        })?;
        ...
        for (key, bucket_events) in buckets {
            ...
            let matched = condition.is_none_or(|condition| condition.matches(count));
    ...
    fn process_ordered_temporal<'a>(
        ...
        for (key, mut bucket_events) in buckets {
            ...
            bucket_events.sort_by_key(|a| a.timestamp);
